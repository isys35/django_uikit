...
from .exceptions import UIkitError
from .forms import (
    FORM_GROUP_CLASS,
    render_field,
    render_form_group, is_widget_with_placeholder, render_label,
)
from .text import text_value
from django.utils.safestring import mark_safe

from .uikit import get_uikit_setting
...
class FieldRenderer(BaseRenderer):
    ...
    def __init__(self, field, *args, **kwargs):
        if not isinstance(field, BoundField):
            raise UIkitError('Parameter "field" should contain a valid Django BoundField.')
        self.field = field
        ...
        elif get_uikit_setting("set_placeholder"):
        ...
        if error_css_class is not None:
            ...
            self.error_css_class = getattr(field.form, "error_css_class", get_uikit_setting("error_css_class"))
        if required_css_class is not None:
            self.required_css_class = required_css_class
        else:
            self.required_css_class = getattr(
                field.form, "required_css_class", get_uikit_setting("required_css_class")
            )
        if bound_css_class is not None:
            self.success_css_class = bound_css_class
        else:
            self.success_css_class = getattr(field.form, "bound_css_class", get_uikit_setting("success_css_class"))
    ...
    def add_class_attrs(self, widget=None):
        ...
        if ReadOnlyPasswordHashWidget is not None and isinstance(widget, ReadOnlyPasswordHashWidget):
            ...
            classes = add_css_class(classes, "uk-input", prepend=True)
            # For these widget types, add the size class here
        elif isinstance(widget, CheckboxInput):
            classes = add_css_class(classes, "uk-checkbox", prepend=True)
        elif isinstance(widget, Select):
            classes = add_css_class(classes, "uk-select", prepend=True)
    ...
    def list_to_class(self, html, classes):
        mapping = [
    ...
    def get_label_class(self):
        ...
        if not label_class:
            label_class = "uk-form-label"
        if not self.show_label or self.show_label == "uk-hidden":
            label_class = add_css_class(label_class, "uk-hidden")
        return label_class
...
class InlineFieldRenderer(FieldRenderer):
    ...
    def get_label_class(self):
        return add_css_class(self.label_class, "uk-hidden")